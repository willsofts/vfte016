...
<template>
  ...
  <div class="pt-page pt-page-current pt-page-controller search-pager">
    <PageHeader ref="pageHeader" :labels="labels" pid="vfte016" version="1.0.0" showLanguage="true" @language-changed="changeLanguage" :multiLanguages="multiLanguages" :build="buildVersion" :visible="displayPageHeader" />
    ...
    this.$nextTick(async () => {
      ...
      startApplication("vfte016",(data) => {
        ...
        if(data.type=="language") {
          ...
          loadAndMergeLabel("vfte016", (success) => {
            if (success) {
  ...
  methods: {
    ...
    dataSelected(item,action) {
      ...
      if("edit"==action) {
        ...
        this.$refs.entryForm.startDeleteRecord({username: item.username, userid: item.userid});
      }
    },
    dataInsert(filters) {
      //listen action from search form
